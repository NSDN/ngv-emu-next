...
namespace NSGDX {
    ...
    void NSGDX::loadNSGAL() {
        ...
        funcList["gal.run"] = $OP_{
            ...
            while (1) {
                funcList["pmq"](nullptr, nullptr, nullptr);
                reg.s = "f1"; funcList["key"](&tmp, &reg, nullptr);
                val = tmp.n.i;
                reg.s = "f8"; funcList["key"](&tmp, &reg, nullptr);
                val += tmp.n.i;
                if (val > 1) break;
                ...
                if (nsgal.m.count(_back) != 0) {
                    ...
                    funcList["eval"](&nsgal.m[_back], nullptr, nullptr);
                }
                ...
                if (
                    ...
                    if (nowScene.m[reg].m.size() > 0) {
                        ...
                        for (
                            ...
                            funcList["eval"](&nsgal.m[_icon], nullptr, nullptr);
                        }
                    }
                }
                    ...
                    if (nowScene.m[reg].m.size() > 0) {
                        ...
                        for (
                            ...
                            if (it->second.m.count(reg) != 0) {
                                ...
                                funcList["back"](&color, nullptr, nullptr);
                            }
                            reg.s = "textFore";
                            if (it->second.m.count(reg) != 0) {
                                color = it->second.m[reg];
                                funcList["fore"](&color, nullptr, nullptr);
                            }

                            funcList["eval"](&nsgal.m[_char], nullptr, nullptr);

                            if (nsgal.m.count(_defback) != 0)
                                funcList["back"](&nsgal.m[_defback], nullptr, nullptr);
                            if (nsgal.m.count(_deffore) != 0)
                                funcList["fore"](&nsgal.m[_deffore], nullptr, nullptr);
                        ...
                        while (1) {
                            ...
                            reg.s = "lu"; funcList["kup"](&tmp, &reg, nullptr);
                            if (tmp.n.i > 0) ptrPos -= 1;
                            reg.s = "ld"; funcList["kup"](&tmp, &reg, nullptr);
                            if (tmp.n.i > 0) ptrPos += 1;
                            if (ptrPos > textCnt - 1) ptrPos = textCnt - 1;
                            if (ptrPos < 0) ptrPos = 0;
                            
                            reg.s = "ru"; funcList["kup"](&tmp, &reg, nullptr);
                            if (tmp.n.i > 0) {
                                tmp.n.i = ptrPos;
                                reg.s = "textJump";
